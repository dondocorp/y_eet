    ...
    def evaluate(
        ...
        result.add(
            CheckResult(
                ...
                if error_rate > self._t.max_error_rate
                else Verdict.PASS,
                message=(
                    f"Error rate {error_rate:.2%} "
                    f"vs threshold {self._t.max_error_rate:.2%}"
                ),
        ...
        result.add(
            CheckResult(
                ...
                verdict=Verdict.FAIL
                if p99 > self._t.p99_latency_ms
                else Verdict.WARN
                if p99 > self._t.p95_latency_ms
                else Verdict.PASS,
                message=(
                    f"p99 latency {p99:.0f}ms "
                    f"vs threshold {self._t.p99_latency_ms:.0f}ms"
                ),
                observed=p99,
        ...
        for ep_name, em in snap.items():
            ...
            if em.timeout_rate > self._t.max_timeout_rate:
                result.add(
                    CheckResult(
                        ...
                        message=(
                            f"{ep_name}: timeout rate "
                            f"{em.timeout_rate:.2%} > threshold"
                        ),
            ...
            if (
                em.total > 0
                and em.auth_failures / em.total > self._t.max_auth_failure_rate
            ):
                result.add(
                    CheckResult(
                        ...
                        verdict=Verdict.WARN,
                        message=(
                            f"{ep_name}: auth failure rate "
                            f"{em.auth_failures / em.total:.2%}"
                        ),
            ...
            if (
                em.trace_sent > 10
                and em.trace_propagation_rate < self._t.min_trace_propagation_rate
            ):
                result.add(
                    CheckResult(
                        ...
                        verdict=Verdict.WARN,
                        message=(
                            f"{ep_name}: trace propagation "
                            f"{em.trace_propagation_rate:.0%} "
                            f"< threshold "
                            f"{self._t.min_trace_propagation_rate:.0%}"
                        ),
            ...
            for mr in mesh_results:
                ...
                result.add(
                    CheckResult(
                        name=f"mesh:{mr.name}",
                        verdict=Verdict.FAIL
                        if mr.status == CheckStatus.FAIL
                        else Verdict.WARN
                        if mr.status == CheckStatus.WARN
                        else Verdict.PASS,
                        message=mr.message,
                    )
                )
            ...
            for cr in chaos_results:
                result.add(
                    CheckResult(
                        ...
                        message=cr.note
                        or f"Expected {cr.expected_status}, got {cr.status_code}",
