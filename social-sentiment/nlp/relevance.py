...
Decision pipeline:
  ...
  4. Secondary only        → score = 0.35, BORDERLINE (flagged irrelevant unless embedding confirms)
    ...
    def classify(self, text: str) -> RelevanceResult:
        ...
        if excl_hits:
            return RelevanceResult(
                is_relevant=False, score=0.0, method="keyword",
        ...
        if prim_hits:
            ...
            score = max(0.6, score)
            return RelevanceResult(
                is_relevant=True, score=round(score, 3), method="keyword",
                matched_primary=prim_hits, matched_secondary=sec_hits,
                matched_context=ctx_hits, derived_labels=derived,
            )
        ...
        if sec_hits and ctx_hits:
            ...
            return RelevanceResult(
                is_relevant=True, score=round(score, 3), method="keyword",
                matched_secondary=sec_hits, matched_context=ctx_hits,
            ...
            if self._embedder is not None:
                ...
                if emb_score >= RELEVANCE_EMBEDDING_THRESHOLD:
                    return RelevanceResult(
                        ...
                        method="hybrid",
                        matched_secondary=sec_hits, derived_labels=derived,
                    )
            return RelevanceResult(
                is_relevant=False, score=round(base_score, 3), method="keyword",
