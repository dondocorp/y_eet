...
import json
import pytest
from pipeline.aggregate import aggregate_hour, detect_spikes
...
def _seed_sentiment_data(db_path, posts: list[dict]) -> None:
    ...
    try:
        ...
        with transaction(db_path) as conn:
            conn.execute(
                "INSERT OR IGNORE INTO scrape_runs (run_id, platform, query, started_at, status) "
                "VALUES ('agg-run-001','reddit','yeet casino','2024-01-15T14:00:00Z','success')"
            )
            for p in posts:
                conn.execute(
                    ...
                    "VALUES (?,?,?,?,?)",
                    (p["post_id"], p["platform"], "agg-run-001", p["text"], p["posted_at"]),
                )
                conn.execute(
                ...
                conn.execute(
                    ...
                    "SELECT np.id, np.platform, np.post_id, ?, ?, ?, ?, ?, ?, np.posted_at "
                    "FROM normalized_posts np WHERE np.post_id=? AND np.platform=?",
                    (
...
def test_aggregate_basic(tmp_db):
    ...
    posts = [
        {"post_id": "a1", "platform": "reddit", "text": "love yeet casino",
         "posted_at": "2024-01-15T14:30:00Z", "label": "positive", "score": 0.9},
        {"post_id": "a2", "platform": "reddit", "text": "yeet casino scam",
         "posted_at": "2024-01-15T14:45:00Z", "label": "negative", "score": 0.85,
         "derived_labels": ["scam_concern"]},
        {"post_id": "a3", "platform": "reddit", "text": "ok yeet casino ok",
         "posted_at": "2024-01-15T14:50:00Z", "label": "neutral", "score": 0.6},
    ]
    try:
...
def test_aggregate_derived_label_counts(tmp_db):
    ...
    sdb.DB_PATH = tmp_db
    posts = [
        {"post_id": f"b{i}", "platform": "twitter", "text": f"post {i}",
         "posted_at": "2024-01-15T14:30:00Z", "label": "negative",
