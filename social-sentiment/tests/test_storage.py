...
def test_init_db_creates_tables(tmp_db):
    ...
    conn.close()
    required = {
        "scrape_runs", "raw_posts", "normalized_posts",
        "classifier_runs", "sentiment_results",
        "hourly_aggregates", "alert_events",
    }
...
def test_insert_and_fetch_raw_posts(tmp_db, sample_posts):
    ...
    conn = db.get_connection(tmp_db)
    conn.execute(
        "INSERT INTO scrape_runs (run_id, platform, query, started_at, status) VALUES (?,?,?,?,?)",
    ...
    sdb.DB_PATH = tmp_db
    try:
        inserted = db.insert_raw_posts(sample_posts, )
        assert inserted == 3
