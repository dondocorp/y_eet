...
from storage.db import fetch_hourly_aggregates, upsert_hourly_aggregate, utcnow, transaction
...
def aggregate_hour(hour_bucket: str, platform: str, brand_query: str) -> Optional[dict]:
        ...
        if platform == "ALL":
            rows = conn.execute(
                """SELECT sr.sentiment_label, sr.sentiment_score,
                   ...
                   AND sr.posted_at >= ?
                   AND sr.posted_at <  datetime(?, '+1 hour')""",
    ...
    avg_score    = sum(r["sentiment_score"] for r in scored) / len(scored) if scored else None
...
def detect_spikes(brand_query: str, platform: str = "ALL") -> list[dict]:
    ...
    from config.settings import ALERT_MENTION_SPIKE_MULTIPLIER, ALERT_NEG_RATIO_THRESHOLD
    if median_mentions > 0 and current_mentions >= median_mentions * ALERT_MENTION_SPIKE_MULTIPLIER:
        anomalies.append({
            "type":          "mention_spike",
            "platform":      platform,
            "brand_query":   brand_query,
